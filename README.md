...
- **Classement anonyme (admin)** : sous le bouton de clôture, `computeLiveRanking()` calcule le score courant de chaque livre à partir des bulletins déjà soumis (même formule que la clôture : `(moyenne+médiane)/2` en tour 1, nombre de voix en tour 2) et affiche un classement **par rang uniquement — jamais le titre du livre**, pour empêcher Tom de se servir du panneau admin pour clôturer stratégiquement quand son livre préféré est en tête. Un indicateur 🔒/⏳ dit si le résultat est **mathématiquement verrouillé** : pire cas pour le livre en tête (chaque votant restant lui donne la pire note/ne vote pas pour lui) comparé au meilleur cas de chaque concurrent (chaque votant restant lui donne la meilleure note/vote pour lui) — si le pire cas du leader dépasse encore le meilleur cas de tous les autres, le résultat ne peut plus changer et clôturer maintenant est sans risque.
