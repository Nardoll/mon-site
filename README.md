...
- **LEC Summer 2026 créé** (doc `jktxjultvQzfzBlKIpkj`, statut `upcoming`) : 24 juillet → 20 septembre 2026, les 10 équipes vérifiées (Fnatic, G2, GIANTX, Karmine Corp, Vitality, Heretics, Shifters, Movistar KOI, SK Gaming, Natus Vincere), `format: 'league'`, `wiki_url` vers la page Leaguepedia. **Les 45 matchs de la saison régulière sont seedés** (IDs `lec2026summer_wN_XX_YY`) depuis la capture du calendrier Leaguepedia fournie par Tom : semaine 1 = week-end parisien à 4 équipes (G2/MKOI/VIT/KC, 24–26 juil), semaines 2–5 = 8 matchs Ven→Lun, semaine 6 = 7 matchs avec triple le dimanche 30 août. Tous en BO3, horaires saisis en heure de Paris, `round`/`tab` = `Semaine N`. Round-robin validé par script (45 paires uniques, 9 matchs/équipe).
